...
describe('cross-service order saga (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    const admin = { name: 'Saga Admin', email: `saga-admin-${unique}@test.local`, password: 'SagaAdmin!2026' };
    const customer = {
      name: 'Saga Customer',
    ...
    const product = await api<{ id: string }>('/api/v1/products', {
      ...
      body: { name: `Saga Widget ${unique}`, sku: `SAGA-${unique}`, price: 42, stockQty: 10 },
    });
    productId = product.json.data!.id;
  });

  it('places an order and reaches PAID through the full event chain', async () => {
    const created = await api<{ id: number; status: string }>('/api/v1/orders', {
      method: 'POST',
      token: customerToken,
    ...
    const payment = await api<{ status: string }>(`/api/v1/orders/${orderId}/payment`, { token: customerToken });
    expect(payment.json.data!.status).toBe('SUCCEEDED');

    const notifications = await api<{ items: Array<{ type: string }> }>('/api/v1/notifications', {
      token: customerToken,
    });
    expect(notifications.json.data!.items.some((n) => n.type === 'ORDER_PLACED')).toBe(true);
    expect(notifications.json.data!.items.some((n) => n.type === 'PAYMENT_SUCCEEDED')).toBe(true);
  });
});
